...
8. 【必须】在回答末尾附上"参考文献"章节。格式要求：
   - 列出你参考或引用的具体来源（论文标题、书籍名称、官方文档链接等）
   - 如果是学术内容，给出完整的文献引用（作者、年份、期刊、DOI）
   - 如果是技术内容，给出官方文档或权威教程的链接
   - 如果某些知识来自你的训练数据而非特定来源，注明"基于模型训练知识"
   - 不要编造不存在的文献，宁可注明"无特定来源"也不要伪造引用`;
